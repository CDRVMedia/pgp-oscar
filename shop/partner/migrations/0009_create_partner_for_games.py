...
class Migration(DataMigration):
    depends_on = [('game', '0032_auto__chg_field_game_slug__add_unique_game_slug')]

    def forwards(self, orm):
        "Write your forwards methods here."
        # create a partner record for each game
        for game in orm['game.Game'].objects.all():
            name = '%s: %s' % (game.developer.name, game.name)
            orm['partner.partner'].objects.create(game=game,
                                                  name=name,
                                                  code=slugify(name))

    def backwards(self, orm):
        orm['partner.partner'].objects.all().delete()
